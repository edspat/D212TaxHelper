...
function buildLocalBreakdown(yd) {
  ...
  // Dividends are split per-country source (D-9):
  //   - foreignSource=false ("RO ISIN" — country='RO' bucket): broker
  //     withheld 8% RO at source, D205 cat 20 IS expected.
  //   - foreignSource=true (any non-RO country): paid by a foreign issuer,
  //     intermediary RO broker did not necessarily withhold RO tax, so
  //     D205 typically does NOT contain this. Surface as
  //     "expected — not in D205" instead of as an unmatched-local warning.
  //   - country=null (parser couldn't resolve): treat as foreign for now
  //     because that's the most common case (XTB descriptions usually
  //     mention a foreign country); the user can override per-bucket from
  //     the UI.
  const splitDividends = (broker, src, byCountry, totalGross, totalTax) => {
    if (!byCountry || byCountry.length === 0) {
      // No per-country detail. Fall back to a single foreign-source row
      // so behavior matches the pre-D-9 state.
      if (totalGross > 0 || totalTax > 0) {
        out.push({ broker, category: '20', label: 'Dividende', grossRON: totalGross, taxRON: totalTax || 0, source: src, foreignSource: true });
      }
      return;
    }
    ...
    if (localGross > 0 || localTax > 0) {
      out.push({ broker, category: '20', label: 'Dividende RO', grossRON: localGross, taxRON: localTax, source: `${src} (RO)`, foreignSource: false });
    }
    if (foreignGross > 0 || foreignTax > 0) {
      out.push({ broker, category: '20', label: 'Dividende externe', grossRON: foreignGross, taxRON: foreignTax, source: `${src} (foreign)`, foreignSource: true });
    }
  };
  ...
  if (yd.xtbDividendsReport) {
    ...
    if (d.dividends && d.dividends.grossRON > 0) {
      splitDividends('XTB', 'xtbDividendsReport.dividends', d.dividendsByCountry,
        d.dividends.grossRON, d.dividends.taxWithheldRON || 0);
    }
  ...
  // BT Capital Partners dividends (Fișă Dividende — piețe externe + interne).
  if (yd.btDividendsReport) {
    const d = yd.btDividendsReport;
    if (d.dividends && d.dividends.grossRON > 0) {
      splitDividends('BT Capital Partners', 'btDividendsReport.dividends', d.dividendsByCountry,
        d.dividends.grossRON, d.dividends.taxWithheldRON || 0);
    }
  }
