...
const RULES = [
  ...
  },
  {
    id: 'pfa-income-tax',
    category: 'other',
    title: 'Impozit pe venit PFA — activități independente (sistem real)',
    taxRates: [
      '2023-2025: 10% pe venit impozabil',
      '2026+: 10% (rata nemodificată de OUG 156/2024 pentru PFA art. 64-67 — verifică acte normative ulterioare)',
    ],
    lawArticle: 'Cod fiscal art. 64-67 (PFA), art. 118 (CASS deductibilă)',
    instructionParagraph: 'Instr. D212 cap I §4.1-4.2 + § 7.3.2',
    formula: [
      'CASS_deductibilă = min(CASS_PFA_datorat, venit_net × 10%)',
      '                  /* D212 cap I §4.2 rd.6 — capata la CASS pe venit real */',
      'CAS_deductibilă  = pondere × CAS_datorat (curent 0 dacă nu modelăm CAS)',
      'venit_impozabil   = max(0, venit_net − CAS_deductibilă − CASS_deductibilă)',
      'impozit_PFA       = venit_impozabil × cota',
    ],
    narrative:
      'Pentru PFA în sistem real, baza de calcul a impozitului pe venit (10%) este venitul net DUPĂ DEDUCEREA CONTRIBUȚIILOR SOCIALE deductibile. CASS deductibilă este plafonată la 10% din venitul net real (nu se folosește CASS calculată pe plafonul ales). Pentru a evita dublul beneficiu fiscal pe baza opt-in art. 180(2).',
    example: {
      input: 'Venit net PFA = 23.498 RON, CASS PFA opt-in = 2.430 RON (la baza 24.300).',
      output:
        'CASS_deductibilă = min(2.430, 23.498 × 10%) = min(2.430, 2.350) = 2.350\nvenit_impozabil = 23.498 − 2.350 = 21.148\nimpozit_PFA = 21.148 × 10% = 2.115 RON',
    },
    codeRef: 'public/js/app.js:2099-2115 (pfaCassDeductible + pfaIncomeTax)',
    seeAlso: ['cass-pfa-separate', 'pfa-opt-in-cass'],
    lastVerified: '2026-05-26',
  },
  {
    id: 'pfa-opt-in-cass',
    category: 'cass',
    title: 'CASS PFA — opțiune sub plafon 6 SM (art. 180(2))',
    taxRates: ['Bază forțată: 6 SM dacă opt-in (= 24.300 RON pentru 2025)'],
    lawArticle: 'Cod fiscal art. 170 alin. (1) lit. a), art. 180 alin. (2)',
    instructionParagraph: 'Instr. D212 cap I §3 subsec 2.1, opțiune declarată',
    formula: [
      'IF venit_net_PFA < 6 SM (= 24.300 RON pentru 2025):',
      '  ┌── pfaOptInCASS = false  → CASS PFA = 0 (sub prag, nu se datorează)',
      '  └── pfaOptInCASS = true   → CASS PFA = 6 SM × 10% (opt-in art. 180(2))',
      'IF venit_net_PFA ≥ 6 SM:',
      '  CASS PFA = tier(venit) × 10%   /* 3-tier 6/12/24 SM */',
    ],
    narrative:
      'Persoanele fizice care realizează venituri PFA sub plafonul de 6 SM pot ALEGE să plătească CASS la baza minimă (6 SM) prin exercitarea opțiunii din D212. Opțiunea trebuie depusă la termen (25 mai a anului următor) și NU este automată — se bifează explicit pe formular. Aplicația oferă un checkbox pentru a reflecta această alegere. Atenție: CASS deductibilă pentru calculul impozitului pe venit este în continuare plafonată la 10% × venit_real (NU la 10% × bază opt-in), pentru a evita dublul beneficiu fiscal.',
    codeRef: 'public/js/app.js:2080-2090 (pfaOptInCASS branch in calculateCASS)',
    seeAlso: ['cass-pfa-separate', 'pfa-income-tax'],
    lastVerified: '2026-05-26',
  },
  {
